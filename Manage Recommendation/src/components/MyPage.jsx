...
function Mypage({ user, setUser }) {
    ...
    const fetchMangaHistory = useCallback(async () => {
        ...
        try {
            const response = await axios.get(
                `http://127.0.0.1:5000/api/manga-history/${user.user_id}`
            );
    ...
    const fetchRecommendations = useCallback(async () => {
        ...
        try {
            const response = await axios.get(
                `http://127.0.0.1:5000/api/recommend-manga/${user.user_id}`
            );
            if (response.status === 200) {
    ...
    const handleDeleteFromHistory = async (mangaTitle) => {
        ...
        try {
            const response = await axios.delete(
                "http://127.0.0.1:5000/api/delete-manga-history",
                {
                    data: {
            ...
            if (response.status === 200) {
                setMangaHistory(
                    mangaHistory.filter(
                        (item) => item.manga_title !== mangaTitle
                    )
                );
